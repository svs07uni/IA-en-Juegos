moveUtils = function () {
    ...
    function flee(target, point){
        var desired = Phaser.Point.subtract(point, target);
        return desired;
    }
    ...
    function fleeSteer(target, position, curVelocity, maxForce, maxSpeed){
        var flee = this.flee(target, position).normalize();
        var desired = new Phaser.Point(flee.x * maxSpeed, flee.y * maxSpeed);
        var steer = MovementUtils.limit(Phaser.Point.subtract(desired, curVelocity), maxForce);
        return steer;
    }
